...
def my_calippso(spheres, tolerances, volume,dim):
    ...
    while(converged == False):
        ...
        for i in range(num_spheres):
                    for j in ((neighbours_lists[i])):
                        if i<j:
                            ...
                            #Define constraints
                            constraint_expr = -2* np.dot(centre_dist ,var_diff) + gamma*(radius_diff)**2 
                            # Finally, add the constraint to the model
                            model.addConstr(constraint_expr <=  np.linalg.norm(centre_dist)**2, name=f"{i,j}")
        ...
        if (math.sqrt(optimal_gamma) - 1 <= 0.000000000001) or (max_Si <= 0.001):
